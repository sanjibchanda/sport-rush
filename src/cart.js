...
function renderCartItems(products) {
  ...
  const cartSelections = JSON.parse(localStorage.getItem('cartSelections')) || {};
  ...
  container.innerHTML = products.map(product => {
    const selection = cartSelections[product.id] || { color: 'Default', size: 'Default' };
    ...
    return `
      <div class="flex gap-4 py-4 border-b border-border">
        <div class="bg-surface2 p-3 size-28 rounded-sm">
          <img alt="${product.title}" class="w-full h-full object-contain" src="${product.image}" />
        </div>
        <div class="w-full flex gap-4 justify-between">
          <div class="space-y-1 w-full">
            ...
            <div class="flex gap-2">
              <p class="text-sm"><span class="font-medium">Color:</span> <span>${selection.color}</span></p>
              <p class="text-sm"><span class="font-medium">Size:</span> <span>${selection.size}</span></p>
            </div>
            <p class="font-semibold font-heading">$${product.price.toFixed(2)}</p>
          </div>
          <div class="flex flex-col justify-between items-end">
            <button class="text-muted hover:text-accent remove-cart" data-id="${product.id}">
              <i class="fa-solid fa-trash-can"></i>
            </button>
            <div class="flex items-center gap-3 px-3 py-2 border-2 border-border rounded-full font-medium text-accent quantity-control" data-id="${product.id}">
...
function refreshCartData() {
  loadAllProducts().then(allProducts => {
    const cart = getCart();
    const cartProducts = allProducts.filter(p => cart.includes(p.id.toString()));
